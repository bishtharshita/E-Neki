...
if($result->num_rows>0)
{
    while($row=$result->fetch_assoc())
    {
       ...
       echo '<form action="doner_profileUpdate.php" method="POST" id="myForm">
    <h1><b> Profile<b></h1>
             <div class="border">
    ...
    <div class="first">
        <div class="wrap">
          <label> Name:</label>
            <input type="text" name="name" class="in" value="' . $row["doner_name"] . '" readonly>
            
             </div>

        <div class="wrap">
            <label> Mobile-No:</label>
         <input type="text" name="mno" class="in" value="' . $row["doner_mobno"] . '" readonly>
        </div>

        <div class="wrap">
            <label> Email-Id:</label>
             <input type="text" name="email"  class="in" value="' . $row["doner_emailid"] . '" readonly>
        </div>
        
        <div class="wrap">
          <label> House-No:</label>
           <input type="text" name="hno"  class="in" value="' . $row["doner_house_no"] . '" readonly>
      </div>

      <div class="wrap">
        <label> Locality:</label>
         <input type="text" name="loc"  class="in" value="' . $row["doner_locality"] . '" readonly>
    </div>

    <div class="wrap">
      <label> Landmark:</label>
       <input type="text" name="land"  class="in" value="' . $row["doner_landmark"] . '" readonly>
  </div>
  
  <div class="wrap">
    <label> Pincode:</label>
     <input type="text" name="pin"  class="in" value="' . $row["doner_pincode"] . '" readonly>
</div>

<div class="wrap">
  <label> City:</label>
   <input type="text" name="city"  class="in" value="' . $row["doner_city"] . '" readonly>
</div>

<div class="wrap">
  <label> State:</label>
   <input type="text" name="state" class="in" value="' . $row["doner_state"] . '" readonly>
</div>
...
// Loop through each input element and set the border style
  ...
  function remove()
  {
    ...
    if(res)
    {
      document.getElementById('myForm').action = 'doner_profileDelete.php';
